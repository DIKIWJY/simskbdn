...
export default function FinanceMonitoringTable({
  documents,
  loading,
  onOpenDetail,
  filters = {},
  ...
  const totalPages =
    documents?.total_pages || Math.max(1, Math.ceil(total / limit));
      ...
      {!lockStatus && (filters.status || "") && (
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 px-2.5 py-1 text-[11px] font-semibold text-blue-700 bg-blue-50 border border-blue-100 rounded-lg">
            Filter:{" "}
            {STATUS_FILTERS.find((f) => f.value === filters.status)?.label}
            <button
              onClick={() => set("status", "")}
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                <th className="px-4 py-3 text-center text-[11px] font-bold text-gray-500 uppercase tracking-wider w-12">
                  No
                </th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                docs.map((doc, idx) => (
                  ...
                  >
                    <td className="px-4 py-4 text-sm text-center text-gray-400 tabular-nums">
                      {from + idx}
                    </td>
                    <td className="px-4 py-4">
                      <p className="text-sm font-bold text-blue-600 font-mono leading-tight">
                        {doc.skbdn_number || "—"}
                      </p>
                      <p className="text-[11px] text-gray-400 mt-0.5 truncate max-w-[170px]">
                        {doc.title}
                      </p>
                    </td>
                    <td className="px-4 py-4">
                    ...
                    <td className="px-4 py-4 text-right">
                      <span className="text-sm text-gray-700 tabular-nums">
                        {doc.tonnage ? `${formatNumber(doc.tonnage)} T` : "—"}
                      </span>
                    </td>
                    <td className="px-4 py-4 text-right">
                      <span className="text-sm text-gray-700 tabular-nums">
                        {doc.total_price
                          ? formatRupiah(doc.total_price, { compact: true })
                          : "—"}
                      </span>
                    </td>
        ...
        {total > 0 && (
          <div className="flex items-center justify-between px-5 py-3.5 border-t border-gray-100 bg-gray-50/50">
            <p className="text-xs text-gray-500">
              {from}–{to} dari{" "}
              <strong className="text-gray-800">{total}</strong> Total
            </p>
